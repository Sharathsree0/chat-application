        ...
        <div className='h-full overflow-scroll relative backdrop-blur-lg'>
            ...
            {call.status === "ringing" && call.incoming && (
                <div className="fixed inset-0 bg-black bg-opacity-90 flex flex-col items-center justify-center text-white z-50">
                    <img src={call.incoming.profilePic || assets.avatar_icon} className="w-28 h-28 rounded-full object-cover mb-4 border-4 border-violet-500" />
                    <h2 className="text-xl font-semibold">{call.incoming.callerName}</h2>
                    <p className="mt-2 text-gray-400 text-sm">Incoming {call.incoming.callType} call...</p>
                    {/* Pulse ring animation */}
                    <div className="flex gap-10 mt-10">
                        <div className="flex flex-col items-center gap-2">
                            <button
                                onClick={endCall}
                                className="bg-red-600 w-16 h-16 rounded-full flex items-center justify-center hover:bg-red-500 transition"
                            >
                        ...
                        <div className="flex flex-col items-center gap-2">
                            <button
                                onClick={acceptCall}
                                className="bg-green-600 w-16 h-16 rounded-full flex items-center justify-center hover:bg-green-500 transition animate-pulse"
                            >
                                <svg className="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
